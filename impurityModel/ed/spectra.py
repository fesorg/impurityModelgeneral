...
def simulate_spectra(es, psis, hOp, T, w, delta, delta_2, POS, epsilons,
    ...
    delta_2 : float
        Distance above the real axis (in eV).
        Gives smearing to L2
    POS : float
        POsition from where delta_2 is applied
    epsilons : list
    ...
    dict of dicts representing the projections to apply for the calculations of the RIXS spectra
    """
    ...
    tOpsIPS = getInversePhotoEmissionOperators(nBaths, lhigh)
    tOpsPS = getPhotoEmissionOperators(nBaths, lhigh)
    if rank == 0: print("Inverse photoemission Green's function..")
    gsIPS = getSpectra(n_spin_orbitals, hOp, tOpsIPS, psis, es, w,
                               delta, delta_2, POS, restrictions)
    if rank == 0: print("Photoemission Green's function..")
    gsPS = getSpectra(n_spin_orbitals, hOp, tOpsPS, psis, es, -w, -delta, -delta_2, POS, restrictions)
    gsPS *= -1
    gs = gsPS + gsIPS
    if rank == 0:
    ...
    # Transition operators
    tOpsPS = getPhotoEmissionOperators(nBaths, llow)
    ...
    if (deltaNIXS > 0):
       ...
       gs = getSpectra(n_spin_orbitals, hOp, tOps, psis, es, wLoss,
                            deltaNIXS, delta_2NIXS, POSNIXS, restrictions)
       if rank == 0:
           print('#eigenstates = {:d}'.format(np.shape(gs)[0]))
    ...
    #print('Dipoleoperators= {:d}'.format(np.shape(tOps)[0]))
    if XAS_projectors:
    ...
    if (len(wIn) > 0):
      ...
      gs = getRIXSmap(n_spin_orbitals, hOp, tOpsIn, tOpsOut, psis, es,
                              wIn, wLoss, delta, delta_2, POS, deltaRIXS, restrictions)
      if rank == 0:
          print('#eigenstates = {:d}'.format(np.shape(gs)[0]))
...
def getInversePhotoEmissionOperators(nBaths, l):
    r'''
...
def getPhotoEmissionOperators(nBaths, l):
    r'''
...
def getGreen(n_spin_orbitals, e, psi, hOp, omega, delta, delta_2, POS, krylovSize,
    ...
    delta_2 : float
        Deviation of L2 from real axis
        Brodening parameter
     POS : float
        Position from which the additional broadening is applied
    krylovSize : int
    ...
    omegaP = np.zeros(omega.shape, dtype=np.complex)
    #omegaP[omega>POS] = omega[omega>POS] + 1j*(delta+delta_2) + e
    #omegaP[omega<=POS] = omega[omega<=POS] +1j*delta + e

    omegaP=omega +1j* delta + 1j* delta_2/2 *(1 + 2 / np.pi *np.arctan(omega - POS)) + e
...
def getSpectra(n_spin_orbitals, hOp, tOps, psis, es, w, delta, delta_2, POS,
               restrictions=None, krylovSize=250, slaterWeightMin=1e-7,
    ...
    delta : float
        ...
        Broadening/resolution parameter.
    delta_2 : float
        Broadening of the L2
    POS: float
        Position between L3 and L2 from where the additional broadening is applied
    restrictions : dict
        Restriction the occupation of generated
        product states.
    krylovSize : int
        ...
        for i in get_job_tasks(rank, ranks, range(n)):
            ...
            for t, tOp in enumerate(tOps):
                ...
                g[i][t,:] = normalization**2*getGreen(
                    n_spin_orbitals, e, psiR, hOp, w, delta, delta_2, POS, krylovSize,
...
def getRIXSmap(n_spin_orbitals, hOp, tOpsIn, tOpsOut, psis, es, wIns, wLoss,
               delta1, delta_2, POS, delta2, restrictions=None, krylovSize=150,
                ...
                for iwIn, wIn in enumerate(wIns):
                    ...
                    for tOut,tOpOut in enumerate(tOpsOut):
                        ...
                        gs[iE,tIn,tOut,iwIn,:] = normalization**2*getGreen(
                            n_spin_orbitals, e, psi3, hOp, wLoss, delta2, 0, 0,
