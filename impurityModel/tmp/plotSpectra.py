...
if 'xas' in locals():
    ...
    if 'rixs' in locals():
        ...
        aSum = (wLoss[1]-wLoss[0])*np.sum(rixs,axis=(0,1,3))*scaleFY
        plt.plot(wIn,aSum,'-r',label='FY')
        mask = wLoss < 0.2 
        y = (wLoss[1]-wLoss[0])*np.sum(rixs[:,:,:,mask],axis=(0,1,3))*scaleFY
        plt.plot(wIn,y,'-b',label='quasi-elastic FY')
        print("Save FY spectra to disk...\n")
        tmp = [wIn,aSum,y]
        np.savetxt('XAS-FY.dat',np.array(tmp).T,fmt='%10.6f',
                   header='E  FY-sum  FY-quasi-elastic')
    plt.legend()
