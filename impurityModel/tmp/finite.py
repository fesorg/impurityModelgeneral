...
def eigensystem(n_spin_orbitals, hOp, basis, nPsiMax, groundDiagMode='H_build',
    ...
    elif groundDiagMode == 'H_build':
        if rank == 0: print('Create Hamiltonian matrix...')
        m = len(basis) 
        #h_big ={}
        # Obtain Hamiltonian in matrix format.
        #basis_local = expand_basis(n_spin_orbitals, h_big, hOp, basis, restrictions,parallelization_mode=groundDiagMode)
        #h, basis_index, basis_offset = get_hamiltonian_matrix_from_h_dict(h_big, basis_local, parallelization_mode=groundDiagMode)
        h = get_hamiltonian_matrix(n_spin_orbitals, hOp, basis, mode=groundDiagMode)
        #h = get_hamiltonian_matrix(n_spin_orbitals, hOp, basis)
        if rank == 0: print('<#Hamiltonian elements/column> = {:d}'.format(
            int((len(np.nonzero(h)[0])*ranks)/m)))
        if rank == 0: print('Diagonalize the Hamiltonian...')
        from scipy.sparse.linalg import LinearOperator
        # Note, mv needs to be syncronized, so only use it in eigsh!
        def hlin_mv(v):
           hv = h.dot(v)
           return comm.allreduce(sendobj=hv,op=MPI.SUM)
        hlin = LinearOperator((m,m), matvec=hlin_mv, dtype=np.complex)
        es, vecs = scipy.sparse.linalg.eigsh(hlin, k=nPsiMax, which='SA',tol=eigenValueTol)
        # es, vecs = lanczos(h, k=nPsiMax, tol=eigenValueTol)
        # Sort the eigenvalues and eigenvectors in ascending order.
        indices = np.argsort(es)
        es = np.array([es[i] for i in indices])
        vecs = np.array([vecs[:,i] for i in indices]).T
...
def get_spherical_2_cubic_matrix(spinpol=False,l=2):
    ...
    if l == 1:
        ...
        u = np.zeros((7,7),dtype=np.complex)
        u[0,1]=-1j/np.sqrt(2)
        u[0,5]=1j/np.sqrt(2)
        u[1,0]=np.sqrt(5/16)
        u[1,2]=-np.sqrt(3/16)
        u[1,4]=np.sqrt(3/16)
        u[1,6]=-np.sqrt(5/16)
        u[2,0]=1j*np.sqrt(5/16)
        u[2,2]=1j*np.sqrt(3/16)
        u[2,4]=1j*np.sqrt(3/16)
        u[2,6]=1j*np.sqrt(5/16)
        u[3,3]=1
        u[4,0]=-np.sqrt(3/16)
        u[4,2]=-np.sqrt(5/16)
        u[4,4]=np.sqrt(5/16)
        u[4,6]=np.sqrt(3/16)
        u[5,0]=1j*np.sqrt(3/16)
        u[5,2]=-1j*np.sqrt(5/16)
        u[5,4]=-1j*np.sqrt(5/16)
        u[5,6]=1j*np.sqrt(3/16)
        u[6,1]=1/np.sqrt(2)
        u[6,5]=1/np.sqrt(2)
...
def get_hamiltonian_matrix(n_spin_orbitals, hOp, basis, mode='sparse_MPI'):
    ...
    elif mode == 'sparse_MPI' or mode == 'H_build':
...
def get_hamiltonian_matrix_from_h_dict(h_big, basis_local, parallelization_mode='serial',
                                       mode='sparse'):
    """
    Return Hamiltonian expressed in the provided basis of product states
    in matrix format.
    ...
    """
    # Number of basis states
    basis_offset = np.zeros(ranks+1,dtype=int)
    basis_index = {}
    if (parallelization_mode == 'H_build'):
      # Get the offset and the indices
      for r in range(ranks):
         rbasis = comm.bcast(basis_local, root=r)
         n = len(rbasis)
         for i in range(n):
            basis_index[rbasis[i]] = basis_offset[r]+i
         basis_offset[r+1] = n+basis_offset[r]
    elif (parallelization_mode == 'serial'):
      raise ValueError('serial not supported') # TESTTEST
      #basis_index = {basis[i]:i for i in range(m)}
      #rest = m
      #for r in range(ranks):
      #   n = rest // (ranks-r)  # integer division
      #   basis_offset[r+1] = n+basis_offset[r]
      #   rest = rest - n
      #mybasis = [basis[i] for i in range(basis_offset[rank],basis_offset[rank+1])]

    n = basis_offset[rank+1] - basis_offset[rank] # number of designated basis states
    m = basis_offset[-1]

    #if rank == 0: print('Filling the Hamiltonian...')
    #progress = 0
    if mode == 'dense' and parallelization_mode == 'serial':
        h = np.zeros((m,m),dtype=np.complex)
        for j in range(m):
            #if rank == 0 and progress + 10 <= int(j*100./n):
            #    progress = int(j*100./n)
            #    print('{:d}% done'.format(progress))
        ...
        for j in range(m):
            ...
            for k, v in res.items():
                ...
                col.append(j)
                row.append(basis_index[k])
        h = scipy.sparse.csr_matrix((data,(row,col)),shape=(m,m))
    elif mode == 'sparse' and parallelization_mode == 'H_build':
        #print "rank,m,n,len(basis_local)",rank,"   ",m,n,len(basis_local)
        #print "rank,basis_offset: ",rank,"   ",basis_offset
        #h = scipy.sparse.csr_matrix(([],([],[])),shape=(m,n))
        data = []
        row = []
        col = []
        for j in range(len(basis_local)):
            res = h_big[basis_local[j]]
            for k, v in res.items():
                col.append(j)
                row.append(basis_index[k])
                data.append(v)
        ...
        #    progress = 100
        #    print('{:d}% done'.format(progress))
        h = scipy.sparse.csr_matrix((data,(row,col)),shape=(m,n))
        # Different ranks have information about different basis states.
        # Therefor, need to broadcast and append sparse Hamiltonians
        #for r in range(ranks):
        #    h = h + comm.bcast(hSparse, root=r) # TEST
    else:
        sys.exit("Wrong input parameters")
    return h, basis_index, basis_offset


def expand_basis(n_spin_orbitals, h_big, hOp, basis0, restrictions,
                 parallelization_mode="serial"):
    """
    Return the size of the new basis.
    ...
    basis0 : tuple
        List of product states. (Usually from psi.keys())
    ...
    if parallelization_mode == "H_build":
        basis_known = set()
        basis_new = set(basis0)
        while len(basis_new) > 0 :
            #if rank == 0: print("i=",i,", n=",n)

            # All the states in basis_new will be known after the H update
            basis_known.update(basis_new)

            # Reserve the already calculated local states
            basis_new_local = basis_new.intersection(set(h_big.keys()))
            for r in range(ranks):
                basis_new.difference_update(comm.bcast(basis_new_local, root=r))
            # Distribute the rest
            m = len(basis_new)
            basis_new_list = list(basis_new)
            for state_index in get_job_tasks(rank, ranks, range(m)):
                basis_new_local.add(basis_new_list[state_index])

            # Add the states to basis_local
            basis_local.update(basis_new_local)

            # Calculate the selected states
            basis_newer_local = set()
            for state in basis_new_local:
                # Obtain H|state>
                if state in h_big:
                    res = h_big[state]
                else:
                    ...
                    h_big[state] = res # Update h_big
                basis_newer_local.update(set(res.keys()).difference(basis_known))

            # Add unique elements of basis_newer_local into basis_new
            basis_new = set()
            for r in range(ranks):
                basis_new.update(comm.bcast(basis_newer_local, root=r))
        # Prune h_big to only contain the states in basis_local
        #for state in (set(h_big.keys()).difference(basis_local)):
        #   h_big.pop(state) #TEST
    else:
        sys.exit("Wrong parallelization parameter.")
    return tuple(sorted(basis_local))
...
def expand_basis_and_hamiltonian(n_spin_orbitals, h_big, hOp, basis0,
    ...
    basis_local = expand_basis(n_spin_orbitals, h_big, hOp, basis0, restrictions,
                         parallelization_mode)
    # Obtain Hamiltonian in matrix format.
    h, basis_index, basis_offset = get_hamiltonian_matrix_from_h_dict(
        h_big, basis_local, parallelization_mode= parallelization_mode)
    return h, basis_index, basis_offset
...
def bicgstab(a,x_local,y_local,basis_length,atol = 1e-5):
   r'''
   Calculate x for a.x == y,
   x_local and y_local are local 
   '''
   info = -1
   m = len(x_local)
   n = np.sum(basis_length)

   # Start the bidiagonal cg
   r0 = y_local.copy()
   # remove the initial guess:
   v = a.dot(x_local)
   v_local = np.zeros(m,dtype=np.complex)
   comm.Reduce_scatter(v,v_local,basis_length,op=MPI.SUM)
   r0 -= v_local
   #Check
   rho = alpha = omega = 1
   rhonext_tmp = np.vdot(r0,r0)
   rhonext = comm.allreduce(sendobj=rhonext_tmp,op=MPI.SUM)
   resnorm = np.abs(np.sqrt(rhonext))
   r = r0.copy()
   p = np.zeros(m,dtype=np.complex)
   v_local = np.zeros(m,dtype=np.complex)
   t_local = np.zeros(m,dtype=np.complex)
   dist = np.zeros(2,dtype=np.complex)

   for i in range(10*n):
      beta = (rhonext/rho)*(alpha/omega)
      betaomega = (rhonext/rho)*alpha
      rho = rhonext
      ...
      # Get s
      denom_tmp = np.vdot(r0,v_local)
      denom = comm.allreduce(sendobj=denom_tmp,op=MPI.SUM)
      alpha = rho/denom
      s = r - alpha*v_local

      #Check
      #snorm_tmp = scipy.linalg.norm(s)
      #snorm = comm.allreduce(sendobj=snorm_tmp,op=MPI.SUM)
      #if (snorm < atol):
      #   x_local += alpha*p
      #   #print("Early exit s ",i)
      #   info = 0 
      #   break

      #Get t
      t = a.dot(s)
      comm.Reduce_scatter(t,t_local,basis_length,op=MPI.SUM)

      tnorm_tmp = np.vdot(t_local,t_local)
      omegatnorm_tmp = np.vdot(t_local,s)

      # Distribute
      dist[0] = tnorm_tmp
      dist[1] = omegatnorm_tmp
      disttmp = comm.allreduce(sendobj=dist,op=MPI.SUM)
      tnorm = disttmp[0]
      omegatnorm = disttmp[1]
      omega = omegatnorm/tnorm
      ...
      # Get rhonext
      rhonext_tmp = -omega*np.vdot(r0,t_local)

      #Check 2
      rnorm_tmp = scipy.linalg.norm(r)
...
def bicgstab_linop(a_local,y_local,basis_offset,atol = 1e-5):
   # Use Linear operators
   m = basis_offset[ranks]
   a = scipy.sparse.csr_matrix((a_local.data,basis_offset[rank]+a_local.indices,a_local.indptr), shape=(m,m))
   y = np.zeros(m,dtype=np.complex)
   for r in range(ranks):
       y[basis_offset[r]:basis_offset[r+1]] = comm.bcast(y_local,root = r)
   from scipy.sparse.linalg import LinearOperator
   # Note, amv needs to be synced, so make sure to only call it with *all* ranks.
   def amv(v):
      av = a.dot(v)
      return comm.allreduce(sendobj=av,op=MPI.SUM)
   alin = LinearOperator((m,m), matvec=amv, dtype=np.complex)
   x,info = scipy.sparse.linalg.bicgstab(alin,y)
