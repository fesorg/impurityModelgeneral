...
spherical_jn = lambda n, z: jv(n+1/2,z)*np.sqrt(np.pi/2)/(np.sqrt(z))
import time
...
from .finite import bicgstab,bicgstab_linop
...
def getGreen(n_spin_orbitals, e, psi, hOp, omega, delta, krylovSize,
    ...
    if mode == 'dict':
        ...
        h, basis_index, basis_offset = expand_basis_and_hamiltonian(
            n_spin_orbitals, h_big, hOp, psi.keys(), restrictions,
            parallelization_mode)
        if rank == 0 and (time.time() - t0 > 1.0): 
        ...
        m = len(basis_index)
        n = basis_offset[rank+1] - basis_offset[rank] # Size of the local basis!
        # Unnecessary (and impossible) to find more than m Krylov basis vectors.
        krylovSize = min(krylovSize,m)
        if mode == "dense":
            h = h.toarray()
        ...
        basis_length = tuple(basis_offset[1:]-basis_offset[0:-1])
        v = np.zeros((2,n), dtype=np.complex) # local size
        w = np.zeros(n, dtype=np.complex) # local size
        wp_local = np.zeros(n, dtype=np.complex) # local size
        wp = np.zeros(m, dtype=np.complex) # full size
        # Express psi as a vector
        for ps, amp in psi.items(): 
          if (basis_index[ps] >= basis_offset[rank] and basis_index[ps] < basis_offset[rank+1]):
            v[0,basis_index[ps]-basis_offset[rank]] = amp
        wp = h.dot(v[0,:])
        # Reduce scatter wp into wp_local
        comm.Reduce_scatter(wp,wp_local,basis_length,op=MPI.SUM) 
        # Get local alpha
        alpha_tmp = np.dot(np.conj(wp_local),v[0,:]).real
        # Allreduce alpha
        alpha[0] = comm.allreduce(sendobj=alpha_tmp,op=MPI.SUM)
        w = wp_local - alpha[0]*v[0,:]
        # Construct Krylov states,
        # and more importantly the vectors alpha and beta
        for j in range(1,krylovSize):
            beta_tmp = np.sum(np.abs(w)**2)
            # Allreduce beta
            beta2 = comm.allreduce(sendobj=beta_tmp,op=MPI.SUM)
            beta[j-1] = sqrt(beta2)
            if beta[j-1] != 0:
                v[1,:] = w/beta[j-1]
            else:
            ...
            comm.Reduce_scatter(wp,wp_local,basis_length,op=MPI.SUM) 
            # Get alpha
            alpha_tmp = np.dot(np.conj(wp_local),v[1,:]).real
            # Allreduce alpha
            alpha[j] = comm.allreduce(sendobj=alpha_tmp,op=MPI.SUM)
            w = wp_local - alpha[j]*v[1,:] - beta[j-1]*v[0,:]
            v[0,:] = v[1,:]
    else:
    ...
    if parallelization_mode == 'serial' or parallelization_mode == "H_build":
            ...
            for iE in range(nE):
                ...
                if rank == 0:
                    print("len(psi1), len(basis_index), len(hExcited), "
                          ...
                          len(hExcited)-nTMP)
                    #if (len(hExcited) > nTMP and ranks == 1):
                    #   dumparray(h,len(basis_index),name="Ham_for_cg-" + str(tIn) + "-" + str(iE))
                m = len(basis_index)
                n = basis_offset[rank+1]-basis_offset[rank]
                basis_length = tuple(basis_offset[1:]-basis_offset[0:-1])

                # Express psi1 as a vector
                y_local = np.zeros(n,dtype=np.complex)
                for ps,amp in psi1.items(): 
                  if (basis_index[ps] >= basis_offset[rank] and basis_index[ps] < basis_offset[rank+1]):
                     y_local[basis_index[ps]-basis_offset[rank]] = amp

                # Express psi2 as an (unknown) vector
                x_local = np.zeros(n,dtype=np.complex)
                ax_local = np.zeros(n,dtype=np.complex)
                ...
                for iwIn, wIn in enumerate(wIns):
                    t0 = time.time()
                    # A = (wIn+1j*delta1+e)*\hat{1} - hOp. (Nb: only the local part!)
                    a = scipy.sparse.csr_matrix(
                           ([wIn+1j*delta1+e]*n,(basis_offset[rank]+range(n),range(n))), shape=(m,n)) - h
                    # Find x by solving: a*x = y 
                    # Biconjugate gradient stabilized method.
                    # Pure conjugate gradient does not apply since
                    # it requires a Hermitian matrix.
                    if (ranks == 1 and False):
                       x,info = scipy.sparse.linalg.bicgstab(a,y_local)
                    elif (m < 1000000 and False):
                       # Use Linear operators
                       x,info = bicgstab_linop(a_big,y_local,basis_offset,atol=slaterWeightMin) # TESTTEST THIS DOES NOT WORK!!!
                    else:
                       # the returned x is the full vector (not only local part)
                       info = bicgstab(a,x_local,y_local,basis_length,atol=slaterWeightMin)
                       if (ranks == 1):
                           x = x_local
                       else:
                           for r in range(ranks):
                              x[basis_offset[r]:basis_offset[r+1]] = comm.bcast(x_local,root = r)
                    # Check
                    if info > 0 :
                        print("Rank ", rank,
                    ...
                    tinner += t2 - t1
                if (rank ==0): 
                  print("RIXS time: touter = {}, tinner = {}".format(touter, tinner))
                  sys.stdout.flush()
        ...
        for tIn, tOpIn in enumerate(tOpsIn):
            ...
            for iE in range(nE):
                ...
                g = {}
                if rank == 0: print('Loop over in-coming photon energies...')
                # Loop over in-coming photon energies, unique for each MPI rank CONTINUE HERE!!!
...
def dumparray(a,n,name="data"):
   tmp = np.zeros((n+1,n+1),dtype=np.float32)
   tmp[0,0] = n
   tmp[0,1:] = range(n)
   tmp[1:,0] = range(n)
   tmp[1:,1:] = np.abs(a.toarray())
   print("Hermitian?:", abs(tmp-tmp.transpose()).max()) 
   tmp.tofile("abs-"+name+'.bin')
